...
final class PostProcessorRegistrationDelegate {
	...
	public static void registerBeanPostProcessors(
		...
		//添加后置处理器PostProcessorRegistrationDelegate中的内部类BeanPostProcessorChecker
		//用于检查spring中的bean是否已经执行后置处理器
		...
		/**
		 * 这个方法注册了一个与AOP相关的后置处理器AnnotationAwareAspectJAutoProxyCreator
		 * 只有在配置类上加了@EnableAspectJAutoProxy注解的才会注册此后置处理器
		 * 这个处理器会将实例化后的bean对象做AOP增强代理
		 * 代理模式有jdk动态代理和cglib动态代理
		 */
	...
	private static void invokeBeanDefinitionRegistryPostProcessors(
		...
		//如果使用者没有直接注册一个BeanDefinitionRegistryPostProcessor的对象
		//那么这个集合中只有spring内部自己创建的一个实现类
		//就是ConfigurationClassPostProcessor
		//这个实现类是在调用AnnotationConfigApplicationContext中初始化AnnotatedBeanDefinitionReader时
		//也就是reader时在reader的构造方法中添加到bean工厂中去的
		for (BeanDefinitionRegistryPostProcessor postProcessor : postProcessors) {
			//这里的调用的是ConfigurationClassPostProcessor的postProcessBeanDefinitionRegistry方法
			//这里面会对加了Configuration注解的对象做处理
			//开始包或类的扫描
			//并把扫描到的类转换成bd放到bean工厂里去
	...
	private static void invokeBeanFactoryPostProcessors(
		...
		/**
		 * 如果是从第一次调用的地方进入的
		 * 那么这里传入的是实现BeanDefinitionRegistryPostProcessor的对象
		 * 如果使用者本身没有实现这个接口，那么这里只会传入spring自己实现的一个对象
		 * 也就是在最开始注册进bean工厂的ConfigurationClassPostProcessor对象
		 */
